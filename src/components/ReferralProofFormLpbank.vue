...
async function handleSubmit() {
  ...
  if (imageBlobs.value.length < 2) {
    formError.value = 'Vui lòng tải lên đủ 2 ảnh bằng chứng LPBANK.'
    return
  }
...
<template>
  <div class="bg-[#111726]/80 border border-amber-500/30 rounded-[24px] p-5">
    ...
    <div v-else class="space-y-4">
      <p class="text-[10px] text-amber-300 tracking-widest font-black italic uppercase">GỬI BẰNG CHỨNG LPBANK</p>
      ...
      <!-- Upload ảnh -->
      <div class="space-y-1.5">
        <label class="text-[10px] text-slate-400 tracking-widest not-italic normal-case font-semibold uppercase">Ảnh bằng chứng * (tối thiểu 2, tối đa 6 ảnh)</label>
        <p class="text-[11px] text-amber-400 not-italic normal-case font-medium">Vui lòng upload đủ 2 ảnh theo mẫu trong hướng dẫn.</p>
        ...
        <div class="grid grid-cols-2 gap-2 pt-1">
          <div class="space-y-1">
            <div
              ...
              @click="openZoom(baseUrl + 'images/anh-lpbank1.jpg')">
              <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" :src="baseUrl + 'images/anh-lpbank1.jpg'" />
            ...
            <p class="text-slate-500 text-[9px] not-italic normal-case text-center">Ảnh mẫu 1</p>
          </div>
          <div class="space-y-1">
            <div
              class="w-full aspect-[3/4] rounded-lg overflow-hidden border border-slate-700/50 bg-slate-900 cursor-zoom-in group relative"
      ...
      <button
        ...
        {{ isCompressing ? 'ĐANG XỬ LÝ ẢNH...' : isSubmitting ? `ĐANG TẢI ẢNH ${uploadDone}/${uploadTotal}...` : 'GỬI BẰNG CHỨNG LPBANK 📤' }}
      </button>
    </div>
