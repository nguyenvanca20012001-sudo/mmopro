...
const steps = [
  'Hướng dẫn bạn bè tải APP ABBANK, đăng ký tài khoản và nhập mã giới thiệu như ảnh mẫu bên dưới.',
  'Làm theo 3 ảnh mẫu bên dưới, sau đó chụp lại và gửi bằng chứng.',
]

const sampleImages = [
  { src: baseUrl + 'images/anh-abbank1.jpg', label: 'Ảnh 1' },
  { src: baseUrl + 'images/anh-abbank2.jpg', label: 'Ảnh 2' },
  { src: baseUrl + 'images/anh-abbank4.jpg', label: 'Ảnh 3' },
]
...
<template>
  <div class="min-h-screen bg-transparent text-white p-4 md:p-8 font-black italic uppercase text-left relative">
    ...
    <div class="max-w-lg mx-auto space-y-5">
      ...
      <div class="relative bg-gradient-to-br from-amber-900/40 to-yellow-800/20 border-[2px] border-amber-500/60 rounded-[28px] p-6 md:p-8 overflow-hidden shadow-[0_4px_30px_rgba(245,158,11,0.25)]">
        ...
        <div class="flex items-center gap-4 mb-4 relative z-10">
          ...
          <div>
            ...
            <h1 class="text-lg md:text-xl text-white leading-tight tracking-tight">
              ...
              <span class="text-amber-300">ĐĂNG KÝ APP ABBANK</span>
            </h1>
          </div>
        </div>
        ...
        <div class="relative z-10 flex items-center gap-2 bg-amber-900/30 border border-amber-500/30 rounded-2xl px-4 py-3 mb-5">
          <svg class="w-7 h-7 shrink-0 drop-shadow-[0_0_8px_rgba(234,179,8,0.6)]" viewBox="0 0 24 24">
            <defs>
              <linearGradient id="abGold" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" style="stop-color:#fde047"/>
                <stop offset="50%" style="stop-color:#eab308"/>
                <stop offset="100%" style="stop-color:#854d0e"/>
              </linearGradient>
            </defs>
            <circle cx="12" cy="12" r="10" fill="url(#abGold)"/>
            <path d="M12 7v10M9 10h6M9 14h6" stroke="#854d0e" stroke-width="2" stroke-linecap="round"/>
          </svg>
          <div>
            <p class="text-[9px] text-slate-400 not-italic normal-case tracking-widest">Thưởng mỗi lượt giới thiệu hợp lệ</p>
            <p class="text-xl text-yellow-400 leading-none tracking-tighter">{{ REFERRAL_REWARD.toLocaleString('vi-VN') }} <span class="text-sm text-yellow-500">XU</span></p>
          </div>
        </div>

        <div class="relative z-10 bg-amber-900/20 border border-amber-500/25 rounded-2xl px-4 py-3 mb-5">
          <p class="text-[10px] text-slate-400 not-italic normal-case font-medium leading-relaxed">
        ...
        <div class="relative z-10 grid grid-cols-2 gap-3">
          <button
            ...
            <span>Gửi bằng chứng ABBANK</span>
          </button>
          <button
      ...
      <!-- ── Cảnh báo 1 SĐT chỉ đăng ký 1 tài khoản ABBANK ───── -->
      <div class="bg-red-950/40 border border-red-500/40 rounded-2xl px-4 py-3 space-y-1.5">
        <p class="flex items-start gap-2 text-red-300 text-[12px] md:text-sm font-bold not-italic normal-case leading-snug">
          ...
          <span>1 điện thoại chỉ đăng ký được 1 tài khoản ABBANK.</span>
        </p>
        <p class="flex items-start gap-2 text-red-300 text-[12px] md:text-sm font-bold not-italic normal-case leading-snug">
          <span class="shrink-0">⚠️</span>
          <span>Không được đăng ký 2 tài khoản ABBANK trên cùng 1 điện thoại.</span>
        </p>
      </div>
