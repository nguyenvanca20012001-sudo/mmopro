  ...
  try {
    ...
    if (userDoc.exists()) {
      ...
      if (!isBoss && userData.role !== 'admin' && userData.site && userData.site !== 'rapjob') {
        await signOut(auth)
        localStorage.clear()
        throw new Error('Tài khoản này không thuộc MMO PRO. Vui lòng đăng nhập đúng trang web!')
      }
    }
...
<template>
  <div class="min-h-screen bg-[#0f172a] lg:bg-white flex font-sans overflow-y-auto lg:overflow-hidden pb-40 lg:pb-0">
    ...
    <div class="hidden lg:flex lg:w-1/2 bg-[#f8fafc] flex-col items-center justify-center p-12 relative overflow-hidden">
      ...
      <div class="absolute top-12 left-12 flex flex-col items-start text-left">
        <div class="flex items-center gap-2 mb-3">
          <div class="bg-blue-700 text-white font-black px-3 py-1 rounded-md italic text-[10px] shadow-lg shadow-blue-200 uppercase tracking-widest">HỆ THỐNG KIẾM TIỀN</div>
      ...
      <div class="max-w-md relative z-10 animate-float">
        <img src="https://cdni.iconscout.com/illustration/premium/thumb/business-partnership-illustration-download-in-svg-png-gif-file-formats--meeting-shaking-hands-collaboration-agreement-pack-office-work-illustrations-4712534.png" alt="MMO PRO Illustration" class="w-full h-auto drop-shadow-[0_35px_35px_rgba(185,28,28,0.12)]">
      </div>
        ...
        <div class="space-y-6">
          <div class="space-y-2 text-left">
            ...
            <div class="relative group">
              <span class="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-600 transition-colors text-lg">👤</span>
              <input v-model="loginInput" type="text" placeholder="USERNAME HOẶC EMAIL..."
                     class="w-full bg-slate-50 border border-slate-100 rounded-[25px] py-6 pl-16 pr-8 text-slate-700 outline-none focus:border-blue-500 focus:bg-white transition-all font-black placeholder:text-slate-300 italic text-sm shadow-inner" />
            </div>
          </div>
