...
// Chặn truy cập trực tiếp route hướng dẫn khi chưa hoàn thành APP LPBANK PLUS hoặc APP ABBANK
...
const sampleImages = [
  ...
  { src: baseUrl + 'images/anh-abbank3.jpg', label: 'Ảnh 3' },
]
...
<template>
  <div class="min-h-screen bg-transparent text-white p-4 md:p-8 font-black italic uppercase text-left relative">
    ...
    <!-- Khóa: chưa hoàn thành APP ABBANK hoặc LPBANK -->
    <div v-else-if="!isUnlocked" class="max-w-lg mx-auto">
      <div class="bg-[#111726]/80 border border-amber-500/30 rounded-[28px] p-8 text-center space-y-4">
        ...
        <p class="text-slate-300 text-sm font-medium not-italic normal-case leading-relaxed">
          Bạn cần hoàn thành Job VIP <span class="text-yellow-400 font-black">APP ABBANK</span> hoặc <span class="text-yellow-400 font-black">LPBANK</span> trước khi xem hướng dẫn.
        </p>
        <button @click="goToVipJobs"
      ...
      <div class="relative bg-gradient-to-br from-amber-900/40 to-yellow-800/20 border-[2px] border-amber-500/60 rounded-[28px] p-6 md:p-8 overflow-hidden shadow-[0_4px_30px_rgba(245,158,11,0.25)]">
        ...
        <div class="relative z-10 flex items-center gap-2 bg-amber-900/30 border border-amber-500/30 rounded-2xl px-4 py-3 mb-5">
          ...
          <div>
            ...
            <p class="text-xl text-yellow-400 leading-none tracking-tighter">85.000 <span class="text-sm text-yellow-500">XU</span></p>
          </div>
        </div>
